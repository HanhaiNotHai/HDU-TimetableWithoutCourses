                ...
                for k in range(5):
                    for j in range(8):
                        if table[start_row_class_table[j]][k + 2]:
                            ...
                            class_times = re_class_time.finditer(table[start_row_class_table[j]][k + 2])
                            for class_time in class_times:
                                class_time = class_time.group().split('节)')
                                start_class, end_class = map(int, class_time[0][1:].split('-'))
                                ...
                                for start_end_week in start_end_weeks:
                                    ...
                                    if len(start_end_week) == 1:
                                        start_week = end_week = int(start_end_week[0])
                                    else:
                                        start_week, end_week = map(int, start_end_week)
                                    if mark != '':
                                        mark = mark[1]
                                        if mark == '单':
                                            if not start_week & 1:
